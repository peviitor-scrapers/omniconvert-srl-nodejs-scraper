...
const OMNICONVERT_ANAF_RECORD = {
  cui: 31411197,
  name: 'OMNICONVERT SRL',
...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch OMNICONVERT via direct CIF lookup and return company data', async () => {
      ...
      expect(result).toHaveProperty('company', 'OMNICONVERT SRL');
      expect(result).toHaveProperty('cif', '31411197');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('OMNICONVERT SRL');
    });
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 31411197, name: null }));
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(OMNICONVERT_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'OMNICONVERT SRL' }]));
