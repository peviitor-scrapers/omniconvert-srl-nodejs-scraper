...
const baseCompany = {
  ...
  career: ["https://www.omniconvert.com/jobs"],
  lastScraped: "2026-06-17"
};

const baseJob = {
  url: "https://www.omniconvert.com/jobs/en/vacancy/123_en",
...
describe("generateJobsMarkdown", () => {
  describe("company section", () => {
    it("includes company name as h1", () => {
      ...
      expect(md).toContain("# OMNICONVERT SRL");
    });

    it("includes CIF", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("31411197");
    });

    it("includes brand", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("Omniconvert");
    });
    ...
    it("includes website as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://www.omniconvert.com](https://www.omniconvert.com)");
    });

    it("includes career page as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, []);
      expect(md).toContain("[https://www.omniconvert.com/jobs](https://www.omniconvert.com/jobs)");
    });
    ...
    it("omits optional fields when not present", () => {
      const minimal = { id: "31411197", company: "OMNICONVERT SRL" };
  ...
  describe("jobs section", () => {
    ...
    it("includes job URL as markdown link", () => {
      const md = generateJobsMarkdown(baseCompany, [baseJob]);
      expect(md).toContain("[https://www.omniconvert.com/jobs/en/vacancy/123_en]");
    });
    ...
    it("renders multiple jobs", () => {
      const job2 = { ...baseJob, title: "DevOps Engineer", url: "https://www.omniconvert.com/jobs/en/vacancy/456_en" };
    ...
    it("handles job with no optional fields", () => {
      const minimal = { url: "https://www.omniconvert.com/jobs/en/vacancy/999_en", title: "QA Engineer" };
