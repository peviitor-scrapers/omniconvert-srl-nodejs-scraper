...
## Project
OMNICONVERT scraper for peviitor.ro (Node.js, ESM, Jest)
...
- **Only the API parsing logic in `scraper/index.js`** (`scrapeJobs`, `parseAboutPageJobs`, `extractStructuredData`) is OMNICONVERT-specific (cheerio-based). The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/omniconvert-srl-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo peviitor-scrapers/omniconvert-srl-nodejs-scraper --state open`
