...
This scraper extracts job listings from the Omniconvert website and imports them to peviitor.ro.

Target: https://www.omniconvert.com/about/
...
1. **Start with brand** - We know the brand ("Omniconvert")
...
8. **Scrape new jobs** - Extract jobs from the Omniconvert website
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Omniconvert jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from the Omniconvert website (jobs only from Romania)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Omniconvert site (jobs for Romania)
    │
    ▼
...
| `tests/unit/index.test.js` | Unit tests for parseAboutPageJobs, extractStructuredData, mapToJobModel, transformJobsForSOLR |
...
- **Omniconvert (HTML)**: `https://www.omniconvert.com/about/` — parsează HTML cu cheerio (link-uri `/jobs/*` + date structurate JSON-LD)
